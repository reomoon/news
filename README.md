...
Streamlit Cloud에서는 `app.py`(내장 HTTP 서버)가 아니라 `news_app.py`를 실행해야 합니다.
...
- Streamlit Cloud 설정
  - Main file path: `news_app.py`
  - Python dependencies: `requirements.txt`

`news_app.py`는 `@st.cache_data(ttl=300)`을 사용해 5분 캐시 갱신을 수행합니다.

## Vercel 기준 스케줄 전략

Vercel은 서버리스 환경이므로 `app.py`의 무한 루프 스케줄러(`scheduler_loop`)가 상시 유지되지 않습니다.
즉, Vercel에서는 앱 내부 `time.sleep(300)` 방식 대신 아래 중 하나를 사용해야 합니다.

### 권장 1) Vercel Cron + 갱신 API 호출

- `/api/refresh` 같은 엔드포인트를 5분마다 호출
...
- "최근 갱신 시각이 5분 이내면 기존 데이터 사용, 초과하면 재수집"
